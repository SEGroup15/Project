...
factory_site varchar(30) not null default 'deleted',
area varchar(30) not null default 'deleted',
name_typology varchar(30) not null default 'deleted' references typology(name_typology) on delete set default,
...
foreign key(factory_site,area) references site(factory_site,area) on delete set default,
check((type='planned' and estimated_time is not null and description is not null) or (type='ewo') or (type='extra')));



create table pm(
name_material varchar(30) not null references material(name_material),
...
('PAV certification','us1'),
('PAV certification','us2'),
('PAV certification','us5'),
('electrical manteinance','us1'),
('electrical manteinance','us3'),
('electrical manteinance','us4'),
('knowledge of cable type','us1'),
('knowledge of cable type','us3'),
('knowledge of cable type','us5'),
('xyz-type robot knowledge','us1'),
('xyz-type robot knowledge','us2'),
('knowledge of robot workstation 23','us1'),
('knowledge of robot workstation 23','us5'),
('knowledge of robot workstation 23','us4');
...
from activity A,possession_act PA, (select M.username, count(M.name_competence) as mc, (select count(name_competence) as pc from possession_procedure where (name_procedure=proc)) 
from possession_maintainer M,possession_procedure  P where (M.name_competence=P.name_competence and P.name_procedure=proc) group by M.username) as L where(PA.id=A.id and week=1) group by L.username,L.pc,L.mc 
); end; $$;
...
(select sum(minuti) from assigned K, activity F where (giorno='sabato' and K.id=F.id and K.username=L.username and F.week=wik) ) as sabato,
(select sum(minuti) from assigned K, activity F where (giorno='domenica' and K.id=F.id and K.username=L.username and F.week=wik) ) as domenica
from activity A,possession_act PA, (select Z.username, count(M.name_competence) as mc, (select count(name_competence) as pc from possession_procedure where (name_procedure=proc)) 
from possession_maintainer M,possession_procedure  P , users Z where
(M.name_competence!=P.name_competence and P.name_procedure=proc and Z.username!= M.username) group by Z.username) as L where(PA.id=A.id and week=wik and L.username not in (select availability_1.username from availability_1(proc,wik))) group by L.username,L.pc,L.mc ); end; $$;
...
drop function if exists availability_1_ewo;
create or replace function availability_1_ewo(iddi integer, gg varchar , wik int ) returns
table(username varchar, mc bigint, ec bigint, uno bigint, due bigint, tre bigint, quattro bigint, cinque bigint, sei bigint, sette bigint) language plpgsql as $$ 
begin
return query (select L.username, L.mc, L.ec, 
(select sum(minuti)  from assigned K, activity F where (K.fascia=1 and K.id=F.id and K.username=L.username and F.week=wik and K.giorno=gg )) as lunedi,
(select sum(minuti)  from assigned K, activity F where (K.fascia=2 and K.id=F.id and K.username=L.username and F.week=wik and K.giorno=gg) ) as martedi,
(select sum(minuti)  from assigned K, activity F where (K.fascia=3 and K.id=F.id and K.username=L.username and F.week=wik and K.giorno=gg) ) as mercoledi,
(select sum(minuti)  from assigned K, activity F where (K.fascia=4 and K.id=F.id and K.username=L.username and F.week=wik and K.giorno=gg) ) as giovedi,
(select sum(minuti)  from assigned K, activity F where (K.fascia=5 and K.id=F.id and K.username=L.username and F.week=wik and K.giorno=gg) ) as venerdi,
(select sum(minuti)  from assigned K, activity F where (K.fascia=6 and K.id=F.id and K.username=L.username and F.week=wik and K.giorno=gg) ) as sabato,
(select sum(minuti)  from assigned K, activity F where (K.fascia=7 and K.id=F.id and K.username=L.username and F.week=wik and K.giorno=gg) ) as domenica
from ewo_competence S, activity A,(select M.username, count(M.name_competence) as mc, (select count(name_competence) from ewo_competence E where E.id=iddi) as ec
from possession_maintainer M, ewo_competence Y  where (Y.name_competence = M.name_competence) group by M.username) as L where(S.id=A.id and week=wik) group by L.username,L.mc,L.ec); end; $$;

drop function if exists availability_2_ewo;
create or replace function availability_2_ewo(iddi integer, gg varchar , wik int ) returns
table(username varchar, mc bigint, ec bigint, uno bigint, due bigint, tre bigint, quattro bigint, cinque bigint, sei bigint, sette bigint) language plpgsql as $$ 
begin
return query (select * from availability_1_ewo(iddi,gg,wik) union 
			  
			  select L.username, 0, L.ec, 
(select sum(minuti)  from assigned K, activity F where (K.fascia=1 and K.id=F.id and K.username=L.username and F.week=wik and K.giorno=gg )) as lunedi,
(select sum(minuti)  from assigned K, activity F where (K.fascia=2 and K.id=F.id and K.username=L.username and F.week=wik and K.giorno=gg) ) as martedi,
(select sum(minuti)  from assigned K, activity F where (K.fascia=3 and K.id=F.id and K.username=L.username and F.week=wik and K.giorno=gg) ) as mercoledi,
(select sum(minuti)  from assigned K, activity F where (K.fascia=4 and K.id=F.id and K.username=L.username and F.week=wik and K.giorno=gg) ) as giovedi,
...
(select sum(minuti)  from assigned K, activity F where (K.fascia=6 and K.id=F.id and K.username=L.username and F.week=wik and K.giorno=gg) ) as sabato,
(select sum(minuti)  from assigned K, activity F where (K.fascia=7 and K.id=F.id and K.username=L.username and F.week=wik and K.giorno=gg) ) as domenica
from ewo_competence S, activity A, (select M.username, (select count(name_competence) from ewo_competence E where E.id=iddi) as ec
from possession_maintainer M, ewo_competence Y, users Z where (Y.name_competence != M.name_competence and Z.username!= M.username) group by M.username) as L where(S.id=A.id and week=wik and L.username not in (select availability_1_ewo.username from availability_1_ewo(iddi,gg,wik))) group by L.username,L.ec); end; $$;
...
if(ewo = 'ewo') then
	...
	insert into pollame values(avail,interr,est);
	while (est>0) loop
...
create or replace function table_ewo(iddi integer) returns
table(username varchar, mc bigint, ec bigint) language plpgsql as $$ 
begin
return query (select pm.username,count(name_competence) mc,(select count(name_competence) ec from ewo_competence where id=iddi) from possession_maintainer pm where pm.name_competence in (select name_competence from ewo_competence where id=iddi) group by pm.username
); end; $$;
...
grant all privileges on pollame to kek;
